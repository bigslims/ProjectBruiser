...
/*bool boolVar = false;//Toggle
	...
	*/
	...
	/*	toolbarInt = GUI.Toolbar(new Rect(25, 25, 250, 30), toolbarInt, toolbarStrings);
		...
		Rect titleRect = new Rect(Screen.width/2-300,(3*intDivider),(4*(Screen.width/5)),(18*intDivider));
		GUI.skin = thisMetalGUISkin;
		GUI.Label(titleRect, ("Project Bruiser"),GUI.skin.GetStyle("label"));
		if(GUI.Button (new Rect(Screen.width/3 ,(23*intDivider) ,(2*(Screen.width/5)) ,(15*intDivider) ) ,("START GAME") ,GUI.skin.GetStyle("box") ) ){
			...
			//	audio.PlayOneShot(onHoverGuiSound);
				...
				if(brainScript.nextTeamSelect == 0){
					brainScript.offenseVector[arrayPos] = 1;
					brainScript.nextTeamSelect = 1;
				}else if(brainScript.nextTeamSelect == 1){
					brainScript.defenseVector[arrayPos] = 1;
					arrayPos++;
					brainScript.nextTeamSelect = 0;
				}
				if(arrayPos> 0){
